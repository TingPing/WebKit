...
{
    auto fontName = SystemSettingsGLib::singleton().fontName();
    ...
    char* fontString = fontName->utf8().mutableData();
    if (auto* spaceChar = strrchr(fontString, ' '))
        *spaceChar = '\0';
    return String::fromUTF8(fontString);
}
